...
class ChangeLangTextFilter(Filter):
    async def __call__(self, message: Message, session: AsyncSession) -> bool | dict:
        ...
        if message.text not in (btn_fa, btn_en):
            return False
        # Pass the resolved user into the handler to avoid a second DB round-trip
        return {"db_user": user}


@router.message(ChangeLangTextFilter())
async def handle_change_language(message: Message, session: AsyncSession, db_user) -> None:
